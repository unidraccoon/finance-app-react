...
import { ModalFormChange } from "./ModalFormChange";
import { abc } from "../Dashboard/index";
...
const AccountsGroup = (props: any) => {
  ...
  return (
    <div className="account-widget-group">
      ...
      {collapsed &&
        accountsFromGroup.map((account: any) => (
  ...
  return (
    <div className="account-widget-account">
      <div className="account-widget-account__name">
        <Link to="/accounts">{account.name}</Link>
      </div>
      ...
      <ModalFormChange account={account} />
    </div>
  );
};
